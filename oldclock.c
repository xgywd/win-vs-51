...
//uchar code table2[8]={0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07};										/*8位一体数码管 位选编码*/
...
void init0() interrupt 0		//按键的设置.用了外部中断.
{
	flag++;									//在按键的时候设置了一个标志.来标志按了几次.按4次归0
	if(4==flag)
	{
		flag=0;
	}
...
void gettime()            //这是根据刚才的那个变量来获取时间
{
	
	if(20==pulse_num)		//每50秒增加1,50*20 如果增加了20次.就正好 是1秒
	{
		miao++;					//让秒的变量加1
		if(60==miao)			//如果60秒到了,
		{
		...
		pulse_num=0;						//计时器变量归0,重新计算一秒的时间.这回时分秒都有了,差的就是显示了.
		
	}
}

void seting()								//seting函数,就要就是根据外部中断标志来判断现在所要做的行为
{
	uint k=0;
	uchar i,j,h;
	...
	{
		k++;
		key();//就 是它.它一直在扫描.键盘的状态.
		if (!flag)																//如果flag这个标志为0,就退出函数.
		{
			pulse_num=0;
			return;
		...
		if(40==k)
		{
			switch (flag)																//用case 选择flag的值.每个值有不同的动作,这些个判断.与一个重要的函数在一起运行
			{
				case 1:
						j=table1[7];
						h=table1[6];
...
void key()									//key是按键函数.这里面,实现了按键的扫描.和时间的修改.逻辑比较复杂.比较费脑袋
{
...
void show()
{
	...
	table1[7]=table[shi/10];				//table1 这个数组.里存放了数码管想要显示的数字.
	table1[6]=table[shi%10];				//这里主要是把两位数的时间拆分.然后分别放入数组.用下面的循环进行显示
